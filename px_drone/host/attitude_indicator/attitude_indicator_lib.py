...
class ETHERNET_ADAPTER_IMU:
    ...
    def __init__(self):
        ...
        self.sock.bind(('',ETHERNET_ADAPTER_IMU.PORT))
        group = socket.inet_aton(ETHERNET_ADAPTER_IMU.BROADCAST)
        iface = socket.inet_aton(ETHERNET_ADAPTER_IMU.INTERFACE) 
        self.sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, group+iface)
        self.sock.settimeout(0.1)

    def recv(self):
        try:
            payload = self.sock.recv(ETHERNET_ADAPTER_IMU.PAYLOAD_SIZE)
            success = True
        except socket.timeout:
            success = False
        if success:
            # Unpack payload
            payload_fields = struct.unpack(ETHERNET_ADAPTER_IMU.PAYLOAD_FORM, payload)
    ...
    def send(self, payload):
        ...
        return self.sock.sendto(payload, (ETHERNET_ADAPTER_IMU.BROADCAST, ETHERNET_ADAPTER_IMU.PORT))

import sys
if __name__ == "__main__":
    error_msg = lambda : print("ERROR: Invalid arguments expect --recv or --send")
    if len(sys.argv) == 2:
        receiver = ETHERNET_ADAPTER_IMU()
        if (sys.argv[1].strip() == "--recv"):
            ...
            roll_inc = True
            while True:
                payload = struct.pack(ETHERNET_ADAPTER_IMU.PAYLOAD_FORM,yaw,pitch,roll,0xff,0,0,0)
